# Transparent Telegram — LSPosed модуль (диагностическая версия)

**Текущий статус: диагностика, а не рабочий эффект.** Две предыдущих
попытки (флаги+фон окна; затем + `Window.setFormat(PixelFormat.TRANSLUCENT)`)
подтверждённо выполняются на реальном устройстве без единого исключения
(проверено по логам LSPosed: "Hooks installed", "Window prepared",
регулярные "Transparency applied"), но визуального эффекта нет. Поэтому
эта версия вместо очередной догадки один раз (через 1.5с после первого
`onResume`) дампит в лог реальное состояние `Window` и дерева `View` —
флаги, `dimAmount`, фон и альфу каждого View на первых 8 уровнях
иерархии. Смотрите `LSPosed Manager -> Logs`, ищите
`[TransparentTelegram][DIAG]`.

По этому логу будет видно, какой конкретно View рисует непрозрачный
слой — следующий шаг будет точечным патчем именно его, а не
"патчим всё подряд и надеемся".
конкретного APK.
